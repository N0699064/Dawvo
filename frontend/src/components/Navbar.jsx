  ...
  return (
    <nav className="fixed top-0 left-0 right-0 bg-white z-50">
      <div className="px-6 py-4">
        <div className="border-[3px] border-black px-12 py-10 flex justify-between items-center">
            ...
            <div className="flex items-center gap-8">
              {topNavLinks.map((link) => (
                <Link
                  key={link.path}
                  to={link.path}
                  className={`text-sm font-normal tracking-normal transition-all duration-300 hover:text-gray-900 uppercase relative pb-1 ${
                ...
                >
                  {link.name}
                  {location.pathname === link.path && (
                    <span className="absolute bottom-0 left-0 right-0 h-[2px] bg-gray-900"></span>
                  )}
                </Link>
              ))}
            </div>
